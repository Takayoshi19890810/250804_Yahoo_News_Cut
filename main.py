...
def transfer_yahoo_news_from_source_sheet():
    ...
    try:
        ...
        print(f"エラー: 出力先シート「{destination_sheet_name}」へのアクセスまたはデータの取得に失敗しました。シートが存在しない可能性があります。詳細: {e}")
    ...
    if existing_data_in_destination:
        if existing_data_in_destination[0][0] == 'ソース':
            header_exists = True
    ...
    if not header_exists:
        print(f"出力先シートにヘッダーが存在しないため、新規作成します。")
    ...
    for i, row in enumerate(data):
        ...
        try:
            ...
            if isinstance(post_date_raw, str):
                ...
                post_date = post_date.astimezone(datetime.timezone(datetime.timedelta(hours=9)))
            else:
                continue
